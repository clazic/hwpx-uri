#!/usr/bin/env bash
# 크로스 컴파일 — macOS(arm64/intel) + Linux(x64/arm64) + Windows(x64) 바이너리 생성
...
CGO_ENABLED=0 GOOS=linux   GOARCH=amd64 go build -o ../bin/hwpxgen-linux-x64     .
CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build -o ../bin/hwpxgen-linux-arm64   .
